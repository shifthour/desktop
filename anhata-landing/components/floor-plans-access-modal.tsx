...
import { X, Eye, User, Phone, Mail, CheckCircle } from "lucide-react"
import { trackGoogleAdsConversion } from '@/lib/google-ads-tracking'

interface FloorPlansAccessModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
}
...
const setUnifiedAccess = () => {
  if (typeof window !== "undefined") {
    console.log("Setting unified access for floor plans...")
...
const saveLeadData = async (
  ...
  try {
    ...
    if (result.success) {
      console.log("Floor plans lead data saved successfully:", result.data)
    } else {
      console.error("Failed to save floor plans lead data:", result.error)
    }

    return result.success
  } catch (error) {
    console.error("Error saving floor plans lead data:", error)
    return false
  }
}

export default function FloorPlansAccessModal({ isOpen, onClose, onSuccess }: FloorPlansAccessModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const leadSaved = await saveLeadData(formData, "floor_plans", "saved")

    if (leadSaved) {
      console.log("Floor plans lead saved successfully")
      
      // TRACK GOOGLE ADS CONVERSION for floor plans access
      trackGoogleAdsConversion('floor_plans', formData)
    } else {
      console.warn("Failed to save floor plans lead data, but continuing")
    }
    ...
    // Call success callback to unlock floor plans
    setTimeout(() => {
      onSuccess()
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="w-[94vw] max-w-xs mx-3 p-0 overflow-hidden bg-white rounded-2xl border-0 shadow-2xl">
        <DialogTitle className="sr-only">Access Floor Plans</DialogTitle>
        {!isSubmitted ? (
          <>
            {/* Header */}
            <div className="relative bg-gradient-to-r from-[#E67E22] to-[#c96b05] px-6 py-8 text-white">
              ...
              <div className="text-center">
                ...
                <h2 className="text-2xl font-bold mb-2">Access Floor Plans</h2>
                <p className="text-orange-100 text-sm">Enter your details to view detailed floor plans</p>
              </div>
            </div>
          ...
          <div className="px-6 py-12 text-center">
            <div className="mb-6">
              ...
              <p className="text-gray-600">You can now view all floor plans and project details.</p>
            </div>
