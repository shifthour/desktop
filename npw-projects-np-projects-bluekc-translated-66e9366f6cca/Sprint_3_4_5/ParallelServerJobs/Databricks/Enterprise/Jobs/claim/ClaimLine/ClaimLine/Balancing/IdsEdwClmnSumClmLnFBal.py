...
# MAGIC 
# MAGIC 
# MAGIC CALLED BY:          IdsEdwClmnSumClmLnFBalSeq
# MAGIC 
# MAGIC PROCESSING:
...
CLM_LN_F.CLM_LN_ALW_AMT,
CLM_LN_F.CLM_LN_CHRG_AMT,
CLM_LN_F.CLM_LN_PAYBL_AMT

FROM 
{EDWOwner}.CLM_LN_F CLM_LN_F FULL OUTER JOIN {EDWOwner}.B_CLM_LN_F B_CLM_LN_F
...
WHERE 
CLM_LN_F.LAST_UPDT_RUN_CYC_EXCTN_SK = {ExtrRunCycle}
AND
(CLM_LN_F.CLM_LN_ALW_AMT <> B_CLM_LN_F.CLM_LN_ALW_AMT 
OR CLM_LN_F.CLM_LN_CHRG_AMT <> B_CLM_LN_F.CLM_LN_CHRG_AMT 
OR CLM_LN_F.CLM_LN_PAYBL_AMT <> B_CLM_LN_F.CLM_LN_PAYBL_AMT)
...
df_notify_numbered = df_in.withColumn("_rn", row_number().over(Window.orderBy(lit(1))))
df_notify = df_notify_numbered.filter(col("_rn") == 1).select(
    rpad(lit("COLUMN SUM BALANCING IDS - EDW CLM LN F OUT OF TOLERANCE"), 70, " ").alias("NOTIFICATION")
)
...
write_files(
    df_Research,
    f"{adls_path}/balancing/research/IdsEdwClmLnFColumnSumResearch.dat.{RunID}",
    delimiter=",",
    mode="overwrite",
    is_pqruet=False,
    header=False,
