...
# MAGIC -------------------------  -------------------   -------------   -----------------------------------------------------------------------------------------------------------------   -------------------------  -------------------
# MAGIC Kalyan Neelam  2013-12-18  4917 PCMH      Initial Programming                                                                                  Kalyan Neelam   2013-12-27

# MAGIC Extract P_RUN_CYC records for IDS PCMH with a value \"N\" in the  EDW_LOAD_IN where the run cycle is >= to the current run cycle used for extracting.
...
temp_table_name = "STAGING.EdwLoadPMedHomeHierRunCycUpdt_P_RUN_CYC_temp"
drop_sql = f"DROP TABLE IF EXISTS {temp_table_name}"
execute_dml(drop_sql, jdbc_url, jdbc_props)

df_BusinessRules.select(
    "SRC_SYS_CD",
    "SUBJ_CD",
    "TRGT_SYS_CD",
    "RUN_CYC_NO"
