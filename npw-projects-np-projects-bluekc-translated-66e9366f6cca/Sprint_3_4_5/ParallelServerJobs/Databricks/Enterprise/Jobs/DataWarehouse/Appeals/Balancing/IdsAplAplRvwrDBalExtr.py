...
df_IdsAplRvwr = (
    ...
    .option("header", False)
    .option("quote", "\"")
    .schema(schema_idsaplrwvr)
    .csv(f"{adls_path}/balancing/snapshot/IDS_APL_RVWR.uniq")
)

df_joined = df_IdsAplRvwr.alias("Snapshot").join(
    df_hf_cdma_codes.alias("SrcSysCdLkup"),
    F.col("Snapshot.SRC_SYS_CD_SK") == F.col("SrcSysCdLkup.CD_MPPNG_SK"),
    "left"
)

df_transform = df_joined.select(
    F.when(
    ...
    F.col("Snapshot.EFF_DT_SK").alias("APL_RVWR_EFF_DT_SK"),
    F.col("Snapshot.APL_RVWR_NM").alias("APL_RVWR_NM")
)

df_final = df_transform.select(
    F.rpad(F.col("SRC_SYS_CD"), <...>, " ").alias("SRC_SYS_CD"),
    F.rpad(F.col("APL_RVWR_ID"), <...>, " ").alias("APL_RVWR_ID"),
    F.rpad(F.col("APL_RVWR_EFF_DT_SK"), 10, " ").alias("APL_RVWR_EFF_DT_SK"),
    F.rpad(F.col("APL_RVWR_NM"), <...>, " ").alias("APL_RVWR_NM")
)

write_files(
    df_final,
    f"{adls_path}/load/B_APL_RVWR_D.dat",
    delimiter=",",
    mode="overwrite",
    is_pqruet=False,
    header=False,
