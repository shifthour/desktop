...
schema_IdsAplActvty = StructType([
...
df_IdsAplActvty = (
    ...
    .option("delimiter", ",")
    .option("quote", "\"")
    .schema(schema_IdsAplActvty)
    .load(f"{adls_path}/balancing/snapshot/IDS_APL_ACTVTY.uniq")
)

df_joined = df_IdsAplActvty.alias("Snapshot").join(
...
df_enriched = df_joined.select(
    ...
    F.col("Snapshot.SEQ_NO").alias("APL_ACTVTY_SEQ_NO"),
    F.col("Snapshot.ACTVTY_DT_SK").alias("APL_ACTVTY_DT_SK"),
    F.col("Snapshot.APL_RVWR_ID").alias("APL_ACTVTY_APL_RVWR_ID")
)

df_enriched = df_enriched.withColumn(
    "SRC_SYS_CD", F.rpad(F.col("SRC_SYS_CD"), F.lit("<...>"), F.lit(" "))
).withColumn(
    "APL_ID", F.rpad(F.col("APL_ID"), F.lit("<...>"), F.lit(" "))
).withColumn(
    "APL_ACTVTY_DT_SK", F.rpad(F.col("APL_ACTVTY_DT_SK"), F.lit(10), F.lit(" "))
).withColumn(
    "APL_ACTVTY_APL_RVWR_ID", F.rpad(F.col("APL_ACTVTY_APL_RVWR_ID"), F.lit("<...>"), F.lit(" "))
)
...
write_files(
    df_final,
    f"{adls_path}/load/B_APL_ACTVTY_D.dat",
    ",",
    "overwrite",
    False,
    False,
