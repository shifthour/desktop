...
# MAGIC """
# MAGIC JOB NAME:  GlDrugJeIdsWrkdyTotalsExtr
# MAGIC Called by : GLJeIdsWrkdyTotalsExtrSeq
# MAGIC 
# MAGIC 
# MAGIC Process Description: This job will extract the monthly GL Drug detail transactions, join to the IDS crosswalk to assign the correct FNCL LOB and load to JRNL_ENTRY totals output file that will be the source                                   of the Finance monthly Journal Entry total report that will be used for balancing
# MAGIC 
# MAGIC Modifications:                        
...
# MAGIC This extract is required for Finance to balance the results of the Drug JE process that extracts data from SYBASE and IDS and creates the monthly Workday JE file
# MAGIC Extract current GL Drug JE details
...
from pyspark.sql.types import StructType, StructField, StringType, DecimalType, DateType, TimestampType, IntegerType
# COMMAND ----------
...
df_DB2_JRNL_ENTRY_TRANS_CRSWALK = (
    ...
    .option("query", extract_query_ids_walk)
    .load()
)
...
SELECT BUSINESS_UNIT_GL,
       ...
       ACCOUNTING_DT,
       WD_ACCT_NO,
       CASE WHEN DR_CR_CD = 'CR'
            THEN CAST((MONETARY_AMOUNT*-1) AS DECIMAL(19,2))
       ...
       MAP_IN,
       APPL_JRNL_ID,
       CASE WHEN JRNL_LN_REF = ' '
            THEN '0000'
            ELSE SUBSTRING(JRNL_LN_REF,7,4)
       END AS GL_DTL_LOB
FROM {BCBSFINOwner}.GL_DRUG_DTL_BCP
WHERE MAP_IN = 'Y'
  AND MONETARY_AMOUNT <> 0.00
"""
df_Src_GL_DRUG_DTL_BCP = (
    spark.read.format("jdbc")
...
df_GlJeLob = df_JeCrrntLob.select(
    ...
    F.rpad(
        F.when(F.length(F.col("FINALLOB")) == 0, trim(F.col("GL_DTL_LOB"))).otherwise(trim(F.col("FINALLOB"))),
        4, " "
    ).alias("FINALLOB")
)
...
df_FnlJeTtl_pre = df_JeLnSum.filter(F.col("GL_AMT_SUM") != 0.00)
df_FnlJeTtl = df_FnlJeTtl_pre.select(
    ...
    F.rpad(F.col("FINALLOB"),20," ").alias("FINALLOB"),
    F.rpad(F.col("APPL_JRNL_ID"),10," ").alias("APPL_JRNL_ID"),
    F.when(F.col("GL_AMT_SUM").isNull(),0.00).otherwise(
        F.when(F.col("GL_AMT_SUM") > 0.00, F.col("GL_AMT_SUM")).otherwise(0.00)
    ).alias("GL_AMT_SUM_DR"),
    F.when(F.col("GL_AMT_SUM").isNull(),0.00).otherwise(
        F.when(F.col("GL_AMT_SUM") < 0.00, F.col("GL_AMT_SUM")).otherwise(0.00)
    ).alias("GL_AMT_SUM_CR")
...
df_FnlJeTtl_final = df_FnlJeTtl.select(
    "BUSINESS_UNIT_GL",
    "ACCOUNTING_DT",
    "WD_ACCT_NO",
    "WD_CC_ID",
