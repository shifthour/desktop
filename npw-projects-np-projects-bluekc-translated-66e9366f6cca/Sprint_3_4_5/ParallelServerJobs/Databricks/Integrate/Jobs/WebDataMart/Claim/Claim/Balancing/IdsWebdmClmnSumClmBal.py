...
# MAGIC 
# MAGIC 
# MAGIC CALLED BY:          IdsWebdmClmnSumClmBalSeq
# MAGIC 
# MAGIC PROCESSING:
...
extract_query_SrcTrgtRowComp = (
    f"SELECT "
    f"CLM.SRC_SYS_CD, "
    f"CLM.CLM_ID, "
    f"CLM.CLM_CHRG_AMT, "
    f"CLM.CLM_PAYBL_AMT "
    f"FROM {ClmMartOwner}.CLM_DM_CLM CLM "
    f"FULL OUTER JOIN {ClmMartOwner}.B_CLM_DM_CLM B_CLM "
    f"ON CLM.SRC_SYS_CD = B_CLM.SRC_SYS_CD AND CLM.CLM_ID = B_CLM.CLM_ID "
    f"WHERE CLM.LAST_UPDT_RUN_CYC_NO = {ExtrRunCycle} "
    f"AND CLM.CLM_CHRG_AMT <> B_CLM.CLM_LN_TOT_CHRG_AMT "
    f"AND CLM.CLM_PAYBL_AMT <> B_CLM.CLM_PAYBL_AMT"
)
...
df_notify = df_SrcTrgtRowComp.limit(1).select(
    F.lit("COLUMN SUM BALANCING IDS - DATAMART CLM DM CLM OUT OF TOLERANCE").alias("NOTIFICATION")
).withColumn("NOTIFICATION", F.rpad(F.col("NOTIFICATION"), 70, " "))

df_research = df_research.select("SRC_SYS_CD", "CLM_ID", "CLM_CHRG_AMT", "CLM_PAYBL_AMT")
write_files(
    df_research,
    f"{adls_path}/balancing/research/IdsWebdmClmColumnSumResearch.dat.{RunID}",
    ",",
    "overwrite",
    False,
    False,
