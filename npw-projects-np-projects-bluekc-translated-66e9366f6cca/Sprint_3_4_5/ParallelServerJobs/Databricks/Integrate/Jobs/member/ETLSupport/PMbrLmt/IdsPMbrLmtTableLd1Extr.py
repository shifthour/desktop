...
extract_query = """SELECT ENR.SRC_SYS_CD_SK,
       ENR.MBR_UNIQ_KEY,
       MAP1.TRGT_CD,
       MAX(ENR.TERM_DT_SK),
...
df_Trans1 = df_IDSPrevMaxEnr.select(
    ...
    col("PROD_ACCUM_ID").alias("PROD_ACCUM_ID"),  # DataStage expression "extrDrvr.PROD_ACCUM_ID"
    col("TERM_DT_SK").alias("TERM_DT_SK"),       # DataStage expression "extrDrvr.TERM_DT_SK"
    col("AS_OF_DT_SK").alias("AS_OF_DT_SK")      # DataStage expression "extrDrvr.AS_OF_DT_SK"
)

df_PopulateWMaxEnrTerm = df_Trans1.select(
    col("SRC_SYS_CD_SK"),
    col("MBR_UNIQ_KEY"),
    col("PROD_ACCUM_ID"),
    rpad(col("TERM_DT_SK"), 10, " ").alias("TERM_DT_SK"),
    rpad(col("AS_OF_DT_SK"), 10, " ").alias("AS_OF_DT_SK")
)

execute_dml(f"CALL {IDSOwner}.BCSP_TRUNCATE('{IDSOwner}', 'W_MBR_ACCUM')", jdbc_url, jdbc_props)

execute_dml(f"DROP TABLE IF EXISTS STAGING.IdsPMbrLmtTableLd1Extr_PopulateWMaxEnrTerm_temp", jdbc_url, jdbc_props)

df_PopulateWMaxEnrTerm.write.format("jdbc") \
    .option("url", jdbc_url) \
    .options(**jdbc_props) \
    .option("dbtable", "STAGING.IdsPMbrLmtTableLd1Extr_PopulateWMaxEnrTerm_temp") \
    .mode("overwrite") \
    .save()

merge_sql = f"""
MERGE INTO #$IDSOwner#.W_MBR_ACCUM AS T
USING STAGING.IdsPMbrLmtTableLd1Extr_PopulateWMaxEnrTerm_temp AS S
ON (
...
execute_dml(f"CREATE TABLE {IDSOwner}.W_BSD_ACCUM (PROD_SK INTEGER NOT NULL, ACCUM_NO INTEGER NOT NULL, BNF_SUM_DTL_NTWK_TYP_CD_SK INTEGER NOT NULL, BNF_SUM_DTL_TYP_CD_SK INTEGER NOT NULL, BLUEKC_DPLY_IN CHAR(1) NOT NULL, MBR_360_DPLY_IN CHAR(1) NOT NULL, EXTRNL_DPLY_ACCUM_DESC VARCHAR(255) NOT NULL, INTRNL_DPLY_ACCUM_DESC VARCHAR(255) NOT NULL, LMT_AMT DECIMAL(13,2) NOT NULL, STOPLOSS_AMT DECIMAL(13,2) NOT NULL, PROD_ID VARCHAR(20) NOT NULL, PROD_CMPNT_PFX_ID VARCHAR(20) NOT NULL, PROD_CMPNT_EFF_DT_SK CHAR(10) NOT NULL, PROD_CMPNT_TERM_DT_SK CHAR(10) NOT NULL, PROD_CMPNT_TYP_CD_SK INTEGER NOT NULL, PROD_CMPNT_TYP_CD VARCHAR(20) NOT NULL, PRIMARY KEY (PROD_SK))", jdbc_url, jdbc_props)

execute_dml(f"DROP TABLE {IDSOwner}.W_MBR_ACCUM", jdbc_url, jdbc_props)

execute_dml(f"{IDSOwner}.W_MBR_ACCUM", jdbc_url, jdbc_props)