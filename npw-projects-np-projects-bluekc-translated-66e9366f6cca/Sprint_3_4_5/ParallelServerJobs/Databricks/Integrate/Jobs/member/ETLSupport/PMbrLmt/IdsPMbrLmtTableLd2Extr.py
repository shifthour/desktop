...
SELECT 
  ...
  MAX(ENR.TERM_DT_SK),
  CASE 
    WHEN MAX(ENR.TERM_DT_SK) > '{CurrDate}' THEN '{CurrDate}' 
    ELSE MAX(ENR.TERM_DT_SK) 
  END AS Max_End_Dt
FROM {IDSOwner}.MBR_ENR ENR,
     {IDSOwner}.CD_MPPNG MAP1
WHERE ENR.EFF_DT_SK <= '{CurrDate}'
...
df_IDSCurMaxEnrRenamed = (
    df_IDSCurMaxEnr
    .withColumnRenamed("MAX(ENR.TERM_DT_SK)", "TERM_DT_SK")
    .withColumnRenamed("Max_End_Dt", "AS_OF_DT_SK")
)

df_Trans2 = df_IDSCurMaxEnrRenamed.select(
    F.col("SRC_SYS_CD_SK"),
    F.col("MBR_UNIQ_KEY"),
    F.col("TRGT_CD").alias("PROD_ACCUM_ID"),
    F.col("TERM_DT_SK"),
    F.col("AS_OF_DT_SK")
)

execute_dml(
    f"CALL {IDSOwner}.BCSP_TRUNCATE('{IDSOwner}', 'W_MBR_ACCUM')",
    jdbc_url,
    jdbc_props
)

temp_table = "STAGING.IdsPMbrLmtTableLd2Extr_DummyDb2Link2_temp"
execute_dml(
...
df_enriched = (
    ...
    .withColumn("AS_OF_DT_SK", rpad("AS_OF_DT_SK", 10, " "))
)
...
merge_sql = f"""
MERGE {IDSOwner}.W_MBR_ACCUM AS T
...
"""
execute_dml(create_w_bsd_accum_sql, jdbc_url, jdbc_props)
