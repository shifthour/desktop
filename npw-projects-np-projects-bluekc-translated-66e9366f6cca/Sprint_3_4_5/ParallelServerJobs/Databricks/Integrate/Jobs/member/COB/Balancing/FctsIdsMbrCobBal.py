...
# MAGIC 
# MAGIC  
# MAGIC CALLED BY:         FctsIdsMbrCobBalSeq
# MAGIC 
# MAGIC PROCESSING:
...
SELECT 
 m.SRC_SYS_CD_SK AS SRC_SRC_SYS_CD_SK,
 m.MBR_UNIQ_KEY AS SRC_MBR_UNIQ_KEY,
 m.MBR_COB_PAYMT_PRTY_CD_SK AS SRC_MBR_COB_PAYMT_PRTY_CD_SK,
 m.MBR_COB_TYP_CD_SK AS SRC_MBR_COB_TYP_CD_SK,
 m.EFF_DT_SK AS SRC_EFF_DT_SK,
 b.SRC_SYS_CD_SK AS TRGT_SRC_SYS_CD_SK,
 b.MBR_UNIQ_KEY AS TRGT_MBR_UNIQ_KEY,
 b.MBR_COB_PAYMT_PRTY_CD_SK AS TRGT_MBR_COB_PAYMT_PRTY_CD_SK,
 b.MBR_COB_TYP_CD_SK AS TRGT_MBR_COB_TYP_CD_SK,
 b.EFF_DT_SK AS TRGT_EFF_DT_SK
FROM {IDSOwner}.MBR_COB m
FULL OUTER JOIN {IDSOwner}.B_MBR_COB b
  ON m.SRC_SYS_CD_SK = b.SRC_SYS_CD_SK
  AND m.MBR_UNIQ_KEY = b.MBR_UNIQ_KEY
  AND m.MBR_COB_PAYMT_PRTY_CD_SK = b.MBR_COB_PAYMT_PRTY_CD_SK
  AND m.MBR_COB_TYP_CD_SK = b.MBR_COB_TYP_CD_SK
  AND m.EFF_DT_SK = b.EFF_DT_SK
...
df_Notify_temp = df_TransformLogic_rn.filter(
    (F.col("INROWNUM") == 1) & (ToleranceCd == "OUT")
).withColumn(
    "NOTIFICATION",
    F.lit("ROW COUNT BALANCING FACETS - IDS MBR COB OUT OF TOLERANCE")
)
