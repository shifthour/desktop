...
# MAGIC 
# MAGIC 
# MAGIC CALLED BY:     FctsIdsClsPlnBalSeq
# MAGIC 
# MAGIC 
# MAGIC PROCESSING:
...
# MAGIC ----------------------------------      -------------------    -----------------------------------    ---------------------------------------------------------                  ----------------------------------     ---------------------------------    -------------------------   
# MAGIC Parikshith Chada              3/20/2007           3264                              Originally Programmed                                     devlIDS30                     Brent Leland                 08-30-2007
...
extract_query = f"""
SELECT
  CLS_PLN.SRC_SYS_CD_SK AS SRC_SRC_SYS_CD_SK,
  CLS_PLN.CLS_PLN_ID AS SRC_CLS_PLN_ID,
  B_CLS_PLN.SRC_SYS_CD_SK AS TRGT_SRC_SYS_CD_SK,
  B_CLS_PLN.CLS_PLN_ID AS TRGT_CLS_PLN_ID
FROM {IDSOwner}.CLS_PLN CLS_PLN
FULL OUTER JOIN {IDSOwner}.B_CLS_PLN B_CLS_PLN
  ON CLS_PLN.SRC_SYS_CD_SK = B_CLS_PLN.SRC_SYS_CD_SK
  AND CLS_PLN.CLS_PLN_ID = B_CLS_PLN.CLS_PLN_ID
WHERE CLS_PLN.LAST_UPDT_RUN_CYC_EXCTN_SK >= {ExtrRunCycle}
"""
...
df_Notify = (
    df_SrcTrgtComp.withColumn("rownum", F.row_number().over(window_spec))
    .filter((F.col("rownum") == 1) & (F.lit(ToleranceCd) == "OUT"))
    .select(F.lit("ROW COUNT BALANCING FACETS - IDS CLS PLN OUT OF TOLERANCE").alias("NOTIFICATION"))
)
...
write_files(
    ...
    f"{adls_path}/balancing/research/FacetsIdsClsPlnResearch.dat.{RunID}",
    ",",
    "overwrite",
    False,
    False,
