...
# MAGIC 
# MAGIC 
# MAGIC CALLED BY:        FctsIdsGrpBalSeq
# MAGIC 
# MAGIC PROCESSING:
...
# MAGIC ----------------------------------      -------------------    -----------------------------------    ---------------------------------------------------------               ----------------------------------         ---------------------------------        -------------------------   
# MAGIC Parikshith Chada              3/22/2007          3264                              Originally Programmed                                  devlIDS30                          Brent Leland                     08-30-2007
...
SELECT
GRP.SRC_SYS_CD_SK AS SRC_SRC_SYS_CD_SK,
GRP.GRP_ID AS SRC_GRP_ID,
B_GRP.SRC_SYS_CD_SK AS TRGT_SRC_SYS_CD_SK,
B_GRP.GRP_ID AS TRGT_GRP_ID
FROM {IDSOwner}.GRP GRP
FULL OUTER JOIN {IDSOwner}.B_GRP B_GRP
    ON GRP.SRC_SYS_CD_SK = B_GRP.SRC_SYS_CD_SK
    AND GRP.GRP_ID = B_GRP.GRP_ID
WHERE GRP.LAST_UPDT_RUN_CYC_EXCTN_SK >= {ExtrRunCycle}
"""
df_SrcTrgtComp = (
...
if ToleranceCd == 'OUT':
    temp_df = df_SrcTrgtComp.limit(1)
    if temp_df.count() > 0:
        df_Notify = temp_df.select(
            F.lit("ROW COUNT BALANCING FACETS - IDS GRP OUT OF TOLERANCE").alias("NOTIFICATION")
        )
    else:
...
if "NOTIFICATION" in df_Notify.columns:
...
write_files(
    df_Research,
    f"{adls_path}/balancing/research/FacetsIdsGrpResearch.dat.{RunID}",
    delimiter=",",
    mode="overwrite",
    is_pqruet=False,
    header=False,
