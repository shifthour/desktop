...
# MAGIC 
# MAGIC 
# MAGIC CALLED BY:      NabpIdsProviderBalSeq
# MAGIC 
# MAGIC PROCESSING:
...
# MAGIC 
# MAGIC Parikshith Chada               8/20/2007         3264                              Modified the balancing process,                          devlIDS30                Steph Goddard             9/6/07
...
extract_query = f"""
SELECT 
PROV.SRC_SYS_CD_SK AS SRC_SRC_SYS_CD_SK,
PROV.PROV_ID AS SRC_PROV_ID,
B_PROV.SRC_SYS_CD_SK AS TRGT_SRC_SYS_CD_SK,
B_PROV.PROV_ID AS TRGT_PROV_ID
FROM {IDSOwner}.CD_MPPNG MPPNG, {IDSOwner}.PROV PROV
FULL OUTER JOIN {IDSOwner}.B_PROV B_PROV
ON PROV.SRC_SYS_CD_SK = B_PROV.SRC_SYS_CD_SK
AND PROV.PROV_ID = B_PROV.PROV_ID
WHERE PROV.LAST_UPDT_RUN_CYC_EXCTN_SK >= {ExtrRunCycle}
AND PROV.SRC_SYS_CD_SK = MPPNG.CD_MPPNG_SK
...
df_Research = df_SrcTrgtComp.filter(
    F.isnull("SRC_PROV_ID")
    | F.isnull("SRC_SRC_SYS_CD_SK")
    | F.isnull("TRGT_PROV_ID")
...
df_SrcTrgtComp_rn = df_SrcTrgtComp.withColumn("rownum", F.row_number().over(windowSpec))

if ToleranceCd == 'OUTTLRNC':
    df_Notify = df_SrcTrgtComp_rn.filter(F.col("rownum") == 1).select(
        F.rpad(F.lit("ROW COUNT BALANCING NABP - IDS PROV OUT OF TOLERANCE"), 70, " ").alias("NOTIFICATION")
    )
else:
...
write_files(
    df_Research,
    f"{adls_path}/balancing/research/NabpIdsProvResearch.dat.{RunID}",
    ",",
    "overwrite",
    False,
    False,
