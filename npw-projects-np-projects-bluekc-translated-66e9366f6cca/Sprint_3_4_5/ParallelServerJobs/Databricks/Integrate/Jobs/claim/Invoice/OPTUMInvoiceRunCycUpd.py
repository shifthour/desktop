...
# MAGIC ------------------      ----------------------------     --------------------------------------------------------------------------------------------------------
# MAGIC 2008-11-01     Sharon Andrew          Original programming                                      walkthru by Steph Goddard  11/07/2008
...
df_Trans1 = df_Trans1.withColumn("EDW_LOAD_IN", F.rpad(F.col("EDW_LOAD_IN"), 1, " "))
df_Trans1 = df_Trans1.select("SRC_SYS_CD","SUBJ_CD","TRGT_SYS_CD","RUN_CYC_NO","CDM_LOAD_IN","EDW_LOAD_IN")

execute_dml("DROP TABLE IF EXISTS STAGING.OPTUMInvoiceRunCycUpd_P_RUN_CYC_temp", jdbc_url, jdbc_props)
(
    df_Trans1.write
    .format("jdbc")
    .option("url", jdbc_url)
    .options(**jdbc_props)
    .option("dbtable", "STAGING.OPTUMInvoiceRunCycUpd_P_RUN_CYC_temp")
    .mode("append")
    .save()
)
...
USING STAGING.OPTUMInvoiceRunCycUpd_P_RUN_CYC_temp AS S
