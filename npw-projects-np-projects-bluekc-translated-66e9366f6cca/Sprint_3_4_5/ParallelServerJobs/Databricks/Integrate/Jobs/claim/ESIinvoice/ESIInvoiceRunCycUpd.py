...
df_P_RUN_CYC = df_trans1.select(
    ...
    "TRGT_SYS_CD",
    "RUN_CYC_NO",
    rpad("CDM_LOAD_IN", 1, " ").alias("CDM_LOAD_IN"),
    rpad("EDW_LOAD_IN", 1, " ").alias("EDW_LOAD_IN")
)

drop_sql = "DROP TABLE IF EXISTS STAGING.ESIInvoiceRunCycUpd_P_RUN_CYC_temp"
execute_dml(drop_sql, jdbc_url, jdbc_props)

df_P_RUN_CYC.write.format("jdbc") \
    .option("url", jdbc_url) \
    .options(**jdbc_props) \
    .option("dbtable", "STAGING.ESIInvoiceRunCycUpd_P_RUN_CYC_temp") \
    .mode("overwrite") \
    .save()

merge_sql = f"""
MERGE INTO {IDSOwner}.P_RUN_CYC AS T
USING STAGING.ESIInvoiceRunCycUpd_P_RUN_CYC_temp AS S
ON (
