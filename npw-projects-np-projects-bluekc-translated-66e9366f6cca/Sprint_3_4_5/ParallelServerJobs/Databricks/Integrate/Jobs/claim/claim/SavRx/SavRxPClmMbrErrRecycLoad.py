...
# MAGIC 
# MAGIC 
# MAGIC DESCRIPTION :  SavRx Claims Member Matching Error File Load.
# MAGIC                                
# MAGIC 
# MAGIC MODIFICATIONS :
# MAGIC 
...
# MAGIC ------------------                            --------------------         ------------------------          -----------------------------------------------------------------------                   --------------------------------         -------------------------------   ----------------------------      
# MAGIC Jaideep Mankala                   2018-02-26              5828                         Original Programming                                                            IntegrateDev2               Kalyan Neelam          2018-02-27   
# MAGIC \(9)\(9)\(9)\(9)\(9)\(9)
...
schema_SavRx_Error_File = StructType([
...
df_SavRx_Error_File = (
    ...
    .option("quote", '"')
    .option("header", True)
    .schema(schema_SavRx_Error_File)
    .load(f"{adls_path_publish}/external/SAVRX_MbrMatch_Rx_ErrorFile.dat")
)

df_hf_savrx_errclm_rm_dupe = dedup_sort(
    df_SavRx_Error_File,
    ["CLM_ID"],
    [("CLM_ID","A")]
)
...
df_Trn_Clm_Sk_Gen = (
    df_hf_savrx_errclm_rm_dupe
    .withColumn("CLM_SK", GetFkeyClm(SrcSysCd, F.lit(1), trim(F.col("CLM_ID")), F.lit("N")))
    ...
    .withColumn(
        "SRC_SYS_GRP_ID",
        F.when(F.col("SRC_SYS_GRP_ID").isNull(), F.lit(" ")).otherwise(F.col("SRC_SYS_GRP_ID"))
    )
