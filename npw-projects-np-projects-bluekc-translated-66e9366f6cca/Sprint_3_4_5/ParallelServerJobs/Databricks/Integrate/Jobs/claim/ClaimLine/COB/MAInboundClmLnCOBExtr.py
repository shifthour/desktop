...
# MAGIC 
# MAGIC DESCRIPTION:Generates COB Claim Line Extract File to be processed by IDS routines
# MAGIC 
# MAGIC 
# MAGIC MODIFICATIONS:
# MAGIC 
...
# MAGIC                                                                                               USER_DEFN_AMT_1,USER_DEFN_AMT_2 in MAInboundClmLn.

# MAGIC This container is used in:
# MAGIC FctsClmLnCOBExtr FctsClmLnDntlCOBExtr
# MAGIC NascoClmLnCobExtr
# MAGIC BCBSSCClmLnCobExtr
# MAGIC MAInboundClmLnCOBExtr
# MAGIC 
# MAGIC These programs need to be re-compiled when logic changes
# MAGIC Apply business logic
# MAGIC Writing Sequential File to ../key
# MAGIC Extract MA Inbound Claim Line Data from File
# MAGIC """
# COMMAND ----------
...
schema_MAInboundClmLn = StructType([
    ...
    StructField("USER_DEFN_AMT_1", DecimalType(18,2), True),
    StructField("USER_DEFN_AMT_2", DecimalType(18,2), True)
])
...
df_DQ = df_MAInboundClmLn

df_BusinessRules_Condition = df_DQ.where(
    (F.col("CLM_COB_PD_AMT") != F.lit("0.00")) & (F.col("REC_TYPE") == F.lit("CLAIM LINE"))
)

df_BusinessRules_AllCol = df_BusinessRules_Condition.select(
    F.lit(SrcSysCdSk).alias("SRC_SYS_CD_SK"),
    F.col("CLM_ID").alias("CLM_ID"),
    F.col("CLM_LN_SEQ_NO").alias("CLM_LN_SEQ_NO"),
    F.lit("NA").alias("CLM_LN_COB_TYP_CD"),
    ...
    F.concat(F.lit(SrcSysCd), F.lit(";"), F.col("CLM_ID"), F.lit(";"), F.col("CLM_LN_SEQ_NO"), F.lit(";"), F.lit("NA")).alias("PRI_KEY_STRING"),
    F.lit("0").alias("CLM_COB_LN_SK"),
    F.lit(CurrRunCycle).alias("CRT_RUN_CYC_EXCTN_SK"),
    F.lit(CurrRunCycle).alias("LAST_UPDT_RUN_CYC_EXCTN_SK"),
    F.lit("NA").alias("CLM_LN_COB_CAR_PRORTN_CD"),
    F.lit("NA").alias("CLM_LN_COB_LIAB_TYP_CD"),
    F.lit("0.00").alias("COB_CAR_ADJ_AMT"),
    F.col("CLM_COB_ALW_AMT").alias("ALW_AMT"),
    F.lit("0.00").alias("APLD_AMT"),
    ...
    F.lit("0.00").alias("OOP_AMT"),
    F.col("CLM_COB_PD_AMT").alias("PD_AMT"),
    F.lit("0.00").alias("SANC_AMT"),
    F.lit("0.00").alias("SAV_AMT"),
    F.lit("0.00").alias("SUBTR_AMT"),
    F.lit(None).alias("COB_CAR_RSN_CD_TX"),
    F.lit(None).alias("COB_CAR_RSN_TX")
)

df_BusinessRules_Snapshot = df_BusinessRules_Condition.select(
    F.lit(SrcSysCdSk).alias("SRC_SYS_CD_SK"),
...
df_BusinessRules_Transform = df_BusinessRules_Condition.select(
    F.lit(SrcSysCdSk).alias("SRC_SYS_CD_SK"),
    F.col("CLM_ID").alias("CLM_ID"),
    F.col("CLM_LN_SEQ_NO").alias("CLM_LN_SEQ_NO"),
    F.lit("NA").alias("CLM_LN_COB_TYP_CD")
...
df_Transformer_in = df_BusinessRules_Snapshot

df_Transformer_var = df_Transformer_in.withColumn(
    "ClmLnCobTypCdSk",
    F.when(
        F.col("CLM_LN_COB_TYP_CD").substr(F.lit(1), F.lit(1)) == F.lit("*"),
...
df_Transformer = df_Transformer_var.select(
    F.col("SRC_SYS_CD_SK"),
    F.col("CLM_ID"),
    F.col("CLM_LN_SEQ_NO"),
...
df_B_CLM_LN_COB = df_Transformer

df_B_CLM_LN_COB_final = df_B_CLM_LN_COB.select(
    F.col("SRC_SYS_CD_SK"),
    F.rpad(F.col("CLM_ID"), 12, " ").alias("CLM_ID"),
    F.col("CLM_LN_SEQ_NO"),
...
df_ClmLnCobPK_Key = ClmLnCobPK(
    df_BusinessRules_Transform,
    df_BusinessRules_AllCol,
    params_ClmLnCobPK
)

df_MAInboundClmLnCOBExtr = df_ClmLnCobPK_Key.select(
...
write_files(
    df_MAInboundClmLnCOBExtr,
    f"{adls_path}/key/{SrcSysCd}ClmLnCOBExtr.{SrcSysCd}ClmLnCOB.dat.{RunID}",
    delimiter=",",
    mode="overwrite",
    is_pqruet=False,
    header=False,
