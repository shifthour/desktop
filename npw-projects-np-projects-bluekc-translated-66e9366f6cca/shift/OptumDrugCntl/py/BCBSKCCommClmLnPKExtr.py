...
def BCBSKCCommClmLnPKExtrActivities(ctx):
  def dfBCBSKCCommClmLnPKExtr():
    dataflowName = "BCBSKCCommClmLnPKExtr"
    ...
    dataflow = MappingDataFlow(
      ...
      script = f"""parameters{{
        ...
        SrcSysCd as string (toString("MEDTRAK")),
  ...
  artifacts, dataflowName = dfBCBSKCCommClmLnPKExtr()
  activityName = "BCBSKCCommClmLnPKExtr"
  ...
  return artifacts, [activity,
    CustomActivity(
      name = ctx.removeAlias("DSU.HASH.CLEAR\\2;hf_clm_ln_pk_lkup;hf_BCBSKCCommClmLnPkExtr"),
      command = ctx.getRoutineCommand(ctx.removeAlias("DSU.HASH.CLEAR\\2;hf_clm_ln_pk_lkup;hf_BCBSKCCommClmLnPkExtr"), isSubroutine=True),
    ...
    FailActivity(
      ...
      depends_on = [ActivityDependency(activity = activityName,
        ...
        ActivityDependency(activity = ctx.removeAlias("DSU.HASH.CLEAR\\2;hf_clm_ln_pk_lkup;hf_BCBSKCCommClmLnPkExtr"),
        dependency_conditions = [DependencyCondition.COMPLETED])])]

def BCBSKCCommClmLnPKExtr(ctx):
  name = "BCBSKCCommClmLnPKExtr"
  artifacts, activities = BCBSKCCommClmLnPKExtrActivities(ctx)
  pipeline = PipelineResource(
    ...
    parameters = {
      ...
      "SrcSysCd": ParameterSpecification(type="String", default_value="MEDTRAK"),
...
def main():
  ...
  artifacts = (BCBSKCCommClmLnPKExtr(ctx))
