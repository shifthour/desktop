...
public class StatusRevert {
    ...
    private static void extractCookies(WebDriver driver) {
        try {
            Set<Cookie> cookies = driver.manage().getCookies();
            Logs.info("Extracting " + cookies.size() + " cookies from browser...");

            if (cookies.isEmpty()) {
                Logs.warn("No cookies found in browser session!");
                sessionCookie = "";
                return;
            }
            ...
            for (Cookie cookie : cookies) {
                ...
                Logs.info("Cookie: " + cookie.getName() + "=" +
            ...
            Logs.info("Session cookie string built successfully (length: " +
                sessionCookie.length() + " chars)");
        } catch (Exception e) {
            Logs.error("Failed to extract cookies: " + e.getMessage());
            sessionCookie = "";
        }
    }
    ...
    public static int callRevertAPI(String apiUrl, String payload) {
        ...
        try {
            ...
            if (sessionCookie != null && !sessionCookie.isEmpty()) {
                ...
                Logs.info("Added cookies to request (length: " + sessionCookie.length() + " chars)");
                Logs.info("Cookie header: " +
                    (sessionCookie.length() > 100 ?
                        sessionCookie.substring(0, 100) + "..." :
                        sessionCookie));
            } else {
                Logs.warn("No session cookies available for API request!");
            }
