...
public class StatusRevert {
    ...
    public static void enableNetworkCapture(WebDriver driver) {
        try {
            Logs.info("========== ENABLING NETWORK CAPTURE ==========");
            isCapturing = true;
            ...
            String interceptorScript =
                ...
                "    if (this._url && this._url.includes('bulkEditAdmin')) {" +
                "      console.log('[XHR INTERCEPTOR] Captured bulkEditAdmin request!');" +
                "      console.log('[XHR INTERCEPTOR] Method:', this._method);" +
                "      console.log('[XHR INTERCEPTOR] URL:', this._url);" +
                "      console.log('[XHR INTERCEPTOR] Data:', data);" +
                ...
                "    console.log('[FETCH INTERCEPTOR] Fetch called:', urlStr);" +
                "    if (urlStr.includes('bulkEditAdmin')) {" +
                "      console.log('[FETCH INTERCEPTOR] Captured bulkEditAdmin request!');" +
                "      console.log('[FETCH INTERCEPTOR] URL:', urlStr);" +
                "      console.log('[FETCH INTERCEPTOR] Options:', JSON.stringify(options));" +
                ...
                "console.log('[INTERCEPTOR] Network interceptors injected successfully');";

            js.executeScript(interceptorScript);
            Logs.info("JavaScript network interceptors injected successfully.");
            Logs.info("Network capture enabled. Waiting for bulkEditAdmin requests...");
            Logs.info("==============================================");
        } catch (Exception e) {
    ...
    public static void disableNetworkCapture(WebDriver driver) {
        ...
        try {
            Logs.info("========== DISABLING NETWORK CAPTURE ==========");
            ...
            // Retrieve captured requests from browser
            Object capturedRequestsObj = js.executeScript("return window.capturedRequests || [];");

            if (capturedRequestsObj == null) {
                Logs.warn("window.capturedRequests is null!");
                Logs.info("Total FCIDs captured: 0");
                isCapturing = false;
                return;
            }
            ...
            Logs.info("Retrieved " + capturedRequests.size() + " captured requests from browser");

            if (capturedRequests.isEmpty()) {
                Logs.warn("No requests were captured!");
                Logs.info("This could mean:");
                Logs.info("1. The bulkEditAdmin API was not called");
                Logs.info("2. The request completed before interceptor was injected");
                Logs.info("3. The URL doesn't contain 'bulkEditAdmin'");
            }
            ...
            for (Map<String, Object> request : capturedRequests) {
                ...
                Logs.info("---------- Captured Request #" + requestCount + " ----------");
                Logs.info("Type: " + type);
                Logs.info("Method: " + method);
                Logs.info("URL: " + url);
                Logs.info("Timestamp: " + timestamp);
                Logs.info("Data (first 500 chars): " +
                    (data != null ? data.substring(0, Math.min(500, data.length())) : "null"));
                ...
                if (data != null && !data.isEmpty()) {
                    ...
                    if (!fcids.isEmpty()) {
                        ...
                        Logs.warn("No FCIDs found in request data");
                    }
                } else {
                    Logs.warn("Request has no data/body");
                }
            }

            // Clear the captured requests in browser
    ...
    private static List<String> extractFCIds(String postData) {
        ...
        Logs.info("Attempting to extract FCIDs from data...");

        try {
            // Try parsing as JSON first
            if (postData.trim().startsWith("{") || postData.trim().startsWith("[")) {
                JSONObject json = new JSONObject(postData);
                Logs.info("Parsed as JSON object with keys: " + json.keySet());

                // Check for fcId directly
                if (json.has("fcId")) {
                    String fcid = json.getString("fcId");
                    fcids.add(fcid);
                    Logs.info("Found fcId in JSON: " + fcid);
                }

                // Check for array of objects with fcId
                if (json.has("records")) {
                    JSONArray records = json.getJSONArray("records");
                    for (int i = 0; i < records.length(); i++) {
                        JSONObject record = records.getJSONObject(i);
                        if (record.has("fcId")) {
                            String fcid = record.getString("fcId");
                            fcids.add(fcid);
                            Logs.info("Found fcId in records[" + i + "]: " + fcid);
                        }
                    }
                }
            }
        } catch (Exception e) {
            Logs.info("Data is not JSON, using regex extraction...");
        }
        ...
        while (matcher.find()) {
            ...
            if (!fcids.contains(fcid)) {
                fcids.add(fcid);
                Logs.info("Extracted FCID #" + matchCount + " via regex: " + fcid);
            }
        }

        if (fcids.isEmpty()) {
            Logs.warn("No FCID pattern matches found in data!");
            Logs.info("Full data: " + postData);
        } else {
            Logs.info("Total FCIDs extracted: " + fcids.size());
        }
