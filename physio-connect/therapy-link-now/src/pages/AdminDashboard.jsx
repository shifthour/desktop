...
import {
  ...
  Loader2, Search, Ban, ShieldCheck, Clock
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const updatePhysio = useMutation({
    mutationFn: ({ id, data }) => base44.entities.Physiotherapist.update(id, data),
                ...
                {filteredPhysios.map(p => (
                  <div key={p.id} className="bg-white rounded-2xl border border-gray-100 p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {p.status === "pending" && (
                        <>
                          <Button size="sm" onClick={() => updatePhysio.mutate({ id: p.id, data: { status: "approved" } })} className="bg-emerald-600 hover:bg-emerald-700 rounded-lg">
                            <Check className="w-4 h-4 mr-1" /> Approve
                          </Button>
                          <Button size="sm" variant="outline" onClick={() => updatePhysio.mutate({ id: p.id, data: { status: "rejected" } })} className="rounded-lg text-red-600 border-red-200 hover:bg-red-50">
                            <X className="w-4 h-4 mr-1" /> Reject
                          </Button>
                        </>
                      )}
                      {p.status === "approved" && (
                        <Button size="sm" variant="outline" onClick={() => updatePhysio.mutate({ id: p.id, data: { status: "suspended" } })} className="rounded-lg text-gray-600">
                          <Ban className="w-4 h-4 mr-1" /> Suspend
                        </Button>
                      )}
                      {(p.status === "rejected" || p.status === "suspended") && (
                        <Button size="sm" onClick={() => updatePhysio.mutate({ id: p.id, data: { status: "approved" } })} className="bg-teal-600 hover:bg-teal-700 rounded-lg">
