...
// GET - Fetch contact field configurations for a company
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Error fetching contact field configurations:', error)
...
// POST - Create or update contact field configuration
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Error saving contact field configuration:', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    return NextResponse.json(data)
  } catch (error: any) {
    console.error('POST contact field error:', error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
}

// PUT - Bulk update contact field configurations (e.g., enable/disable, reorder)
export async function PUT(request: NextRequest) {
  try {
    ...
    const updates = fieldConfigs.map(async (config) => {
      ...
      if (error) {
        console.error(`Error updating contact field ${config.field_name}:`, error)
        throw error
      }
    ...
    console.error('PUT contact fields error:', error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
}

// DELETE - Remove contact field configuration (soft delete by disabling)
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Error deleting contact field configuration:', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }
