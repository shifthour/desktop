...
// GET - Fetch product field configurations for a company
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Error fetching product field configurations:', error)
...
// PUT - Bulk update product field configurations (e.g., enable/disable, reorder)
export async function PUT(request: NextRequest) {
  try {
    ...
    console.error('PUT product fields error:', error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
}
